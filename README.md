...
Ce tutoriel vous a permis d'assimiler les bases d'une des fonctionnalités fondamentale que python propose qui est la fonction enumerate(),en effet ce tutoriel vous montre comment parcourir des séquences tout en maintenant une référence d'index associée à chaque élément ,mais aussi l'extraction des données à partir d'un fichier csv,la focntion enumerate() est trés aventageuse car elle facilite la maniplation des données et rend le code plus lisible. elle permet aussi d'éviter les errures qui sont liées à la gestion manuelle des index pendant les itérations sur des structures de données
